...
def cat(*args):
    return tf.keras.layers.Concatenate(axis=-1)(list(args))
...
class InpaintNN:
    ...
    def check_model_file(self):
        if not os.path.exists(self.bar_model_name) or not os.path.exists(
            ...
            print(
                "Read : https://github.com/deeppomf/DeepCreamPy/blob/master/docs"
				"/INSTALLATION.md#run-code-yourself \n")
            exit(-1)
    ...
    def build_model(self):
        ...
        self.MASK = tf.placeholder(tf.float32, [self.batch_size, self.input_height,
                                                self.input_width, 3])
        ...
        I_co = mm.decoder(vec_en, self.input_height, self.input_height, reuse=False,
                          name='G_de')

        self.image_result = I_co * (1 - self.MASK) + self.Y * self.MASK

        D_real_red = self.discriminator(self.Y)
        D_fake_red = self.discriminator(self.image_result)
        ...
        Loss_s_re = tf.reduce_mean(tf.abs(I_co - self.Y))
        ...
        Loss_G = 0.1 * Loss_gan + 5 * (3 - alpha) * Loss_s_re

        # --------------------- variable & optimizer

        var_D = [v for v in tf.global_variables() if v.name.startswith('disc_red')]
        var_G = [v for v in tf.global_variables() if
                 v.name.startswith('G_en') or v.name.startswith(
                     'G_de') or v.name.startswith('CB1')]

        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)

        with tf.control_dependencies(update_ops):
            optimize_D = tf.train.AdamOptimizer(learning_rate=0.0004, beta1=0.5,
                                                beta2=0.9).minimize(Loss_D,
                                                                    var_list=var_D)
            optimize_G = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.5,
                                                beta2=0.9).minimize(Loss_G,
                                                                    var_list=var_G)
    ...
    def predict(self, censored, unused, mask):
        img_sample = self.sess.run(self.image_result,
                                   feed_dict={self.X: censored, self.Y: unused,
                                              self.MASK: mask
                                              })
